...
Stricli's default API uses `this` binding and inline definitions:
...
With stricli-define, you can separate concerns and get better type inference:
...
const handler = defineHandler<typeof params>((ctx, { flags }) => {
